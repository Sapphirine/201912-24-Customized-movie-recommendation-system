...
def rate_movies():
    if request.method=='GET':
        ...
        for i in range(16):
            print(session.get('user_id'), session['id_list'][i],rate_list[i],int(time.time()))
            SQL2="""
                INSERT INTO movie.ratings VALUES (%d, %d, %d, %d)
                """%(int(session.get('user_id')), session['id_list'][i],rate_list[i],int(time.time()))
            print(SQL2)
            pandas_gbq.read_gbq(SQL2)
        print(rate_list)
        session['just_result']=False
        return render_template('message.html',just_rate=True)

@app.route('/result/')
@login_required
def result():
    if session.get('just_result')==True:
        return render_template('result.html',movieTitle=session['movieTitle'],recommendP=session['recommendP'],movieUrl=session['movieUrl'])
    else:
        SQL = """
                SELECT *
                FROM `movie.ratings`
        ...
        recommendP = model.recommendProducts(int(session.get('user_id')),10) # 返回参数 第二个是产品
        ...
        session['just_result']=True
        session['movieTitle']=movieTitle
        session['recommendP']=recommendP
